...
class WebChatModel(ChatModel):
    ...
    def unload_model(self, data) -> Generator[str, None, None]:
        ...
        if self.engine is not None and hasattr(self.engine, "model"):
            model = self.engine.model
            import torch, gc
            print(f"[DEBUG] unload model type: {type(model)}")
            print(f"[DEBUG] VRAM before unload: {torch.cuda.memory_allocated()/1024**3:.2f} GB")
            print(f"[DEBUG] model refcount: {len(gc.get_referrers(model))}")
            if hasattr(model, "base_model"):  # PeftModel wrapping a base model
